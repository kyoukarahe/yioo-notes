...
**완료는 에이전트가 더 할 일이 없다고 말한 상태가 아니다. 의도한 결과가 의도한 진입점에서 도달 가능하고, 필요한 변화가 실제로 남았으며, 그 결과를 판정할 권위 있는 곳에서 다시 확인된 상태다.** 다만 모든 작업을 끝까지 검증할 필요는 없다. 필요한 증거의 높이는 실패의 영향과 되돌리기 난이도가 정한다. 검증을 더 검증하는 일도 새로운 정보를 주는 지점까지만 해야 한다.
